...
class ExamShiftController extends Controller
{
    public function index()
    {
        ...
        $formattedExamShifts = $examShifts->map(function ($examShift) {
            ...
            if ($examShift->exam) {
                $formattedShift['exam'] = [
                    'id' => $examShift->exam->Id,
                    'name' => $examShift->exam->Name,
                    'description' => $examShift->exam->Description,
                    'duration' => $examShift->exam->Duration,
                    'totalQuestions' => $examShift->exam->TotalQuestions,
                    'subject' => null,
                    'grade' => null
                ];
    
                if ($examShift->exam->subjectGrade) {
                    if ($examShift->exam->subjectGrade->subject) {
                        $formattedShift['exam']['subject'] = [
