...
class ExamQuestionController extends Controller
{
    public function index($examId)
    {
        ...
        $examShift = ExamShift::find($examShift);

        if ($examShift) {
            // Trả về ExamId nếu tìm thấy
            return response()->json(['ExamId' => $examShift->ExamId]);
        } else {
            // Trả về lỗi 404 nếu không tìm thấy
            return response()->json(['error' => 'ExamShift not found'], 404);
        }
