...
class QuestionAnswerController extends Controller
{
    ...
    {
        // Lấy giá trị subjectgradeid từ request
        $subjectGradeId = $request->input('subjectgradeid');
    
        // Kiểm tra xem subjectgradeid có được truyền vào không
        if (!$subjectGradeId) {
            return response()->json(['error' => 'SubjectGradeId is required'], 400);
        }
    
        // Lấy tất cả các câu hỏi có subject_grade_id bằng giá trị subjectgradeid từ request
        $questions = QuestionAnswer::where('subject_grade_id', $subjectGradeId)
            ->with('subject_grade')
            ->get();
